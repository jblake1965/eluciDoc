...
while True:
    secTerm: str = input(r'''Enter a predicate search term or phrase - include "'s" or "'" for possessive case of the
     party being searched:''')

    if not secTerm.startswith("'"):  # add a space where the second term is not a possessive
        secTerm = " ".join(["", secTerm])

    search_phrase = f'{party}' + secTerm
    if case_sensitive == 'No':
    ...
    response = pyip.inputMenu(['Enter another search term?', 'Finished'], numbered=True)
